...
function widget_blogger($title)
{
    ...
    <?php
    echo iitboyDice_widget_blogger_div();
}
        ...
        <div id="fenlei_biankuang">
            <ul id="blogsort">
                ...
                foreach ($sort_cache as $value):
                    ...
                    <a href="<?php echo Url::sort($value['sid']); ?>" title="<?php echo $value['sortname']; ?>共有<?php echo $value['lognum'] ?>篇文章"><?php echo $value['sortname']; ?>
                        (<?php echo $value['lognum'] ?>)</a>
                    <?php if (!empty($value['children'])): ?>
                    <ul class="childrenSort">
                        <?php
                        ...
                        foreach ($children as $key):
                            ...
                            ?>
                            <li>
                                <i class="fa fa-level-up fa-flip-horizontal fa-fw"></i>
                                <a href="<?php echo Url::sort($value['sid']); ?>" title="<?php echo $value['sortname']; ?>共有<?php echo $value['lognum'] ?>篇文章"><?php echo $value['sortname']; ?>
                                    (<?php echo $value['lognum'] ?>)</a>
                            </li>
                        <?php endforeach; ?>
            ...
            <div class="wenzhang">
                <ul>
                    <?php foreach ($newLogs_cache as $value) {
                        ?>
                        <li>
                            <i class="fa fa-dot-circle-o"></i><a title="<?php echo $value['title']; ?>" href="<?php echo Url::log($value['gid']); ?>"><?php echo subString(strip_tags($value['title']), 0, 16); ?></a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
...
{
    $index_randlognum = Option::get('index_randlognum') ?: Option::get('index_hotlognum');
    $Log_Model = new Log_Model();
    $randLogs = $Log_Model->getRandLog($index_randlognum); ?>
    <div id="suijiwenzhang">
        <div id="suiji_biaoti"><span><?php echo $title; ?></span></div>
        <div id="suiji_biankuang">
            <div class="wenzhang">
                <ul>
                    <?php foreach ($randLogs as $value): ?>
                        <li>
                            <i class="fa fa-dot-circle-o"></i><a href="<?php echo Url::log($value['gid']); ?>"><?php echo $value['title']; ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
        ...
        <div id="yqlj_link">
            <ul id="link">
                <?php foreach ($link_cache as $value): ?>
                    <li>
                        <i class="fa fa-link fa-fw"></i><a href="<?php echo $value['url']; ?>" title="<?php echo $value['des']; ?>" target="_blank"><?php echo $value['link']; ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
            <div id="gaodu1"></div>
                ...
                <?php if (!empty($value['children'])) : ?>
                    <ul class="sub-nav">
                        <?php foreach ($value['children'] as $row) {
                            echo '<li><a href="' . Url::sort($row['sid']) . '"class="shake shake-little">' . $row['sortname'] . '</a></li>';
                        } ?>
                    </ul>
                <?php endif; ?>
                ...
                <?php if (!empty($value['childnavi'])) : ?>
                    <ul class="sub-nav">
                        <?php foreach ($value['childnavi'] as $row) {
                            ...
                            echo '<li><aclass="shake shake-little" href="' . $row['url'] . "\" $newtab >" . $row['naviname'] . '</a></li>';
                        } ?>
                    </ul>
...
function blog_tag($blogid)
{
    ...
    if (!empty($log_cache_tags[$blogid])) {
        ...
        $tag = '<span class="color">&nbsp;╭(′▽`)╯标签走丢啦~</span>';
        echo $tag;
    }
}
...
function neighbor_log($neighborLog)
{
    ...
    <div id="sxpbk1">
        <div id="wzlj1">
            <a href="<?php echo Url::log($prevLog['gid']) ?>"><i class="fa fa-arrow-circle-left"></i>&ensp;<?php echo $prevLog['title']; ?>
            </a></div>
    </div>
<?php else: ?>
    <div id="sxpbk1">
        <div id="wzlj1"><a rel="prev" class="shake shake-opacity"><i class="fa fa-arrow-circle-left"></i>&ensp;没有上一篇咯，看看别的吧！？</a>
        </div>
    </div>
<?php endif; ?>
    <?php if ($nextLog): ?>
    <div id="sxpbk2">
        <div id="wzlj1">
            <a href="<?php echo Url::log($nextLog['gid']) ?>"></i><?php echo $nextLog['title']; ?>&ensp;<i class="fa fa-arrow-circle-right"></i></a>
        </div>
    </div>
<?php else: ?>
    <div id="sxpbk2">
        <div id="wzlj1">
            <a rel="next" class="shake shake-opacity">没有下一篇咯，看看别的吧！？&ensp;<i class="fa fa-arrow-circle-right"></i></a>
        </div>
    </div>
<?php endif; ?>
<?php } ?>
...
function neighbor_log2($neighborLog)
{
    ...
    <div id="shangxiapian3">
        <a href="<?php echo Url::log($prevLog['gid']) ?>">
            <img src="<?php echo TEMPLATE_URL; ?>images/shang2.png" onmouseover="this.src='<?php echo TEMPLATE_URL; ?>images/shang1.png'" onmouseout="this.src='<?php echo TEMPLATE_URL; ?>images/shang2.png'"></a>
    </div>
<?php else: ?>
...
<?php endif; ?>
    ...
    <div id="shangxiapian4">
        <a href="<?php echo Url::log($nextLog['gid']) ?>"><img src="<?php echo TEMPLATE_URL; ?>images/xia2.png" onmouseover="this.src='<?php echo TEMPLATE_URL; ?>images/xia1.png'" onmouseout="this.src='<?php echo TEMPLATE_URL; ?>images/xia2.png'"></a>
    </div>
<?php else: ?>
        ...
        <div class="comment" id="comment-<?php echo $comment['cid']; ?>">
            ...
            <div class="comment-info">
                ...
                <div class="comment-reply">
                    <a href="#comment-<?php echo $comment['cid']; ?>" onclick="commentReply(<?php echo $comment['cid']; ?>,this)" data-chaffle>回复</a>
                </div>
            </div>
        ...
        <div id="comment-place">
            <div class="comment-post" id="comment-post">
                <div class="cancel-reply" id="cancel-reply" style="display:none">
                    <a href="javascript:void(0);" onclick="cancelReply()">取消回复</a></div>
...
function index_t($num)
{
    ...
    while ($row = $t->fetch_array($list)) {
        ...
        <div id="gonggao_bk">
            <div class="ggwz2"><img src="<?php echo TEMPLATE_URL; ?>images/gonggao_xlb.gif">
                <b>公告：</b><a href="/t"><?php echo $row['content']; ?></a></div>
        </div>
    <?php } ?>
<?php } ?>
...
function checkbaidu($id)
{
    ...
    if (baidu($url) == 1) {
        ...
        if (ROLE == 'admin' || ROLE == 'writer') {
            ...
            $api = 'http://data.zz.baidu.com/urls?site=dxoca.cn&token=mGK65USq2yMVwZEe';
            $ch = curl_init();
            $options = array(
                CURLOPT_URL => $api,
                CURLOPT_POST => true,
